// LOGIN
export const LOGIN_REQUEST = 'LOGIN_REQUEST';
export const LOGIN_SUCCESS = 'LOGIN_SUCCESS';
export const LOGIN_FAIL = 'LOGIN_FAIL';
export const SET_USER_DETAILS = 'SET_USER_DETAILS';

export const USER_VERIFY_SUCCESS = "USER_VERIFY_SUCCESS";
export const USER_VERIFY_FAIL = "USER_VERIFY_FAIL";

// USER
export const USER_LIST_REQUEST = 'USER_LIST_REQUEST';
export const USER_LIST_SUCCESS = 'USER_LIST_SUCCESS';
export const USER_LIST_FAIL = 'USER_LIST_FAIL';

export const USER_ADD_SUCCESS = 'USER_ADD_SUCCESS';
export const USER_ADD_FAIL = 'USER_ADD_FAIL';

export const USER_GET_SUCCESS = 'USER_GET_SUCCESS';
export const USER_GET_FAIL = 'USER_GET_FAIL';

export const USER_UPDATE_SUCCESS = 'USER_UPDATE_SUCCESS';
export const USER_UPDATE_FAIL = 'USER_UPDATE_FAIL';

export const USER_DELETE_SUCCESS = 'USER_DELETE_SUCCESS';
export const USER_DELETE_FAIL = 'USER_DELETE_FAIL';

export const REQUEST_NEW_PASSWORD_SUCCESS = 'REQUEST_NEW_PASSWORD_SUCCESS';
export const REQUEST_NEW_PASSWORD_FAIL = 'REQUEST_NEW_PASSWORD_FAIL';

export const CHANGE_PASSWORD_SUCCESS = 'CHANGE_PASSWORD_SUCCESS';
export const CHANGE_PASSWORD_FAIL = 'CHANGE_PASSWORD_FAIL';

export const USER_LOGOUT = 'USER_LOGOUT';


// CHATROOM
export const CHATROOM_LIST_REQUEST = 'CHATROOM_LIST_REQUEST';
export const CHATROOM_LIST_SUCCESS = 'CHATROOM_LIST_SUCCESS';
export const CHATROOM_LIST_FAIL = 'CHATROOM_LIST_FAIL';

export const CHATROOM_CREATE_REQUEST = 'CHATROOM_CREATE_REQUEST';
export const CHATROOM_CREATE_SUCCESS = 'CHATROOM_CREATE_SUCCESS';
export const CHATROOM_CREATE_FAIL = 'CHATROOM_CREATE_FAIL';

export const ADD_PARTICIPANTS_SUCCESS = 'ADD_PARTICIPANTS_SUCCESS';
export const ADD_PARTICIPANTS_FAIL = 'ADD_PARTICIPANTS_FAIL';

// MESSAGE
export const MESSAGE_LIST_REQUEST = 'MESSAGE_LIST_REQUEST';
export const MESSAGE_LIST_SUCCESS = 'MESSAGE_LIST_SUCCESS';
export const MESSAGE_LIST_FAIL = 'MESSAGE_LIST_FAIL';

export const MESSAGE_CREATE_REQUEST = 'MESSAGE_CREATE_REQUEST';
export const MESSAGE_CREATE_SUCCESS = 'MESSAGE_CREATE_SUCCESS';
export const MESSAGE_CREATE_FAIL = 'MESSAGE_CREATE_FAIL';

// SUBSCRIPTION
export const ADD_SUBSCRIPTION_REQUEST = 'ADD_SUBSCRIPTION_REQUEST';
export const ADD_SUBSCRIPTION_SUCCESS = 'ADD_SUBSCRIPTION_SUCCESS';
export const ADD_SUBSCRIPTION_FAIL = 'ADD_SUBSCRIPTION_FAIL';

export const DELETE_SUBSCRIPTION_REQUEST = 'DELETE_SUBSCRIPTION_REQUEST';
export const DELETE_SUBSCRIPTION_SUCCESS = 'DELETE_SUBSCRIPTION_SUCCESS';
export const DELETE_SUBSCRIPTION_FAIL = 'DELETE_SUBSCRIPTION_FAIL';

// MEETINGROOM
export const MEETINGROOM_ACCESS_REQUEST = 'MEETINGROOM_ACCESS_REQUEST';
export const MEETINGROOM_ACCESS_SUCCESS = 'MEETINGROOM_ACCESS_SUCCESS';
export const MEETINGROOM_ACCESS_FAIL = 'MEETINGROOM_ACCESS_FAIL';

// ✅ Stream Actions
export const START_STREAM_REQUEST = "START_STREAM_REQUEST";
export const START_STREAM_SUCCESS = "START_STREAM_SUCCESS";
export const START_STREAM_FAIL = "START_STREAM_FAIL";

export const STOP_STREAM_REQUEST = "STOP_STREAM_REQUEST";
export const STOP_STREAM_SUCCESS = "STOP_STREAM_SUCCESS";
export const STOP_STREAM_FAIL = "STOP_STREAM_FAIL";

// ✅ WebRTC Capabilities
export const GET_RTP_CAPABILITIES_REQUEST = "GET_RTP_CAPABILITIES_REQUEST";
export const GET_RTP_CAPABILITIES_SUCCESS = "GET_RTP_CAPABILITIES_SUCCESS";
export const GET_RTP_CAPABILITIES_FAIL = "GET_RTP_CAPABILITIES_FAIL";

// ✅ Producer Transport (Broadcaster)
export const CREATE_PRODUCER_TRANSPORT_REQUEST = "CREATE_PRODUCER_TRANSPORT_REQUEST";
export const CREATE_PRODUCER_TRANSPORT_SUCCESS = "CREATE_PRODUCER_TRANSPORT_SUCCESS";
export const CREATE_PRODUCER_TRANSPORT_FAIL = "CREATE_PRODUCER_TRANSPORT_FAIL";

export const CONNECT_PRODUCER_TRANSPORT_REQUEST = "CONNECT_PRODUCER_TRANSPORT_REQUEST";
export const CONNECT_PRODUCER_TRANSPORT_SUCCESS = "CONNECT_PRODUCER_TRANSPORT_SUCCESS";
export const CONNECT_PRODUCER_TRANSPORT_FAIL = "CONNECT_PRODUCER_TRANSPORT_FAIL";

export const PRODUCE_REQUEST = "PRODUCE_REQUEST";
export const PRODUCE_SUCCESS = "PRODUCE_SUCCESS";
export const PRODUCE_FAIL = "PRODUCE_FAIL";

// ✅ Consumer Transport (Viewer)
export const CREATE_CONSUMER_TRANSPORT_REQUEST = "CREATE_CONSUMER_TRANSPORT_REQUEST";
export const CREATE_CONSUMER_TRANSPORT_SUCCESS = "CREATE_CONSUMER_TRANSPORT_SUCCESS";
export const CREATE_CONSUMER_TRANSPORT_FAIL = "CREATE_CONSUMER_TRANSPORT_FAIL";

export const CONNECT_CONSUMER_TRANSPORT_REQUEST = "CONNECT_CONSUMER_TRANSPORT_REQUEST";
export const CONNECT_CONSUMER_TRANSPORT_SUCCESS = "CONNECT_CONSUMER_TRANSPORT_SUCCESS";
export const CONNECT_CONSUMER_TRANSPORT_FAIL = "CONNECT_CONSUMER_TRANSPORT_FAIL";

export const CONSUME_REQUEST = "CONSUME_REQUEST";
export const CONSUME_SUCCESS = "CONSUME_SUCCESS";
export const CONSUME_FAIL = "CONSUME_FAIL";





